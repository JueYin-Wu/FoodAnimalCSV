...
public class AnimalFoodOptimization {
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        for (int i = 0; i < a; i++) {
            ...
            for (int j = 0; j < g; j++) {
                ...
                for (int k = 0; k < fg; k++) {
                    ...
                    FoodItem item = group.getFoodItemByNameAndQuality(LectorCSV.foodItems, name, quality);
                    group.addFood(item);
                }
        ...
        double[] coefficients = new double[LectorCSV.foodItems.size()];
        for (int i = 0; i < LectorCSV.foodItems.size(); i++) {
            coefficients[i] = LectorCSV.foodItems.get(i).quality;
        }
        ...
        // Budget constraint
        double[] priceCoefficients = new double[LectorCSV.foodItems.size()];
        for (int i = 0; i < LectorCSV.foodItems.size(); i++) {
            priceCoefficients[i] = (LectorCSV.foodItems.get(i).minPrice + LectorCSV.foodItems.get(i).maxPrice) / 2;
        }
        constraints.add(new LinearConstraint(priceCoefficients, Relationship.LEQ, budget));

        // Minimum quality constraint
        double[] qualityCoefficients = new double[LectorCSV.foodItems.size()];
        for (int i = 0; i < LectorCSV.foodItems.size(); i++) {
            qualityCoefficients[i] = LectorCSV.foodItems.get(i).quality;
        }
        ...
        for (int i = 0; i < LectorCSV.foodItems.size(); i++) {
            foodIndexMap.put(LectorCSV.foodItems.get(i).name, i);
        }

        for (AnimalRequirement requirement : animalRequirements) {
            int animalCount = requirement.count;
            for (FoodGroup group : requirement.foodGroups) {
                double[] groupCoefficients = new double[LectorCSV.foodItems.size()];
        ...
        if (solution != null) {
            ...
            for (int i = 0; i < LectorCSV.foodItems.size(); i++) {
                System.out.printf("Food item %s: %f units%n", LectorCSV.foodItems.get(i).name, solutionVector.getEntry(i));
            }
        } else {
